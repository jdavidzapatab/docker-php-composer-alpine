...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -t davidzapata/php-composer-alpine:5.6 --push .
```
...
```bash
cd my_dir
docker run --rm -v $(pwd):/var/www davidzapata/php-composer-alpine:5.6 composer create-project --prefer-dist laravel/laravel blog
cd blog
```
...
```bash
docker run --rm -v $(pwd):/var/www davidzapata/php-composer-alpine:5.6 ./vendor/bin/phpunit
```

Or you can serve it using:

```bash
docker run --rm -p 80:80 -v $(pwd):/var/www davidzapata/php-composer-alpine:5.6 php -S 0.0.0.0:80 -t public
```

## Security & CVE Scanning

To ensure the image is as secure as possible while maintaining PHP 5.6, this Dockerfile follows best practices and applies targeted security updates from the Alpine `edge` repository for core packages like `tar`, `curl`, and `busybox`.
...
Using Docker Scout:
```bash
docker build -t davidzapata/php-composer-alpine:5.6 .
docker scout cves davidzapata/php-composer-alpine:5.6
```

Using Trivy:
```bash
trivy image davidzapata/php-composer-alpine:5.6
```

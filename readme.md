...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -t davidzapata/php-composer-alpine:8.1 --push .
```
...
To ensure the image is as secure as possible while maintaining PHP 8.1, this Dockerfile follows best practices and applies targeted security updates from the Alpine `edge` repository for core packages like `tar`, `curl`, and `busybox`.
...
Using Docker Scout:
```bash
docker build -t davidzapata/php-composer-alpine:8.1 .
docker scout cves davidzapata/php-composer-alpine:8.1
```

Using Trivy:
```bash
trivy image davidzapata/php-composer-alpine:8.1
```
